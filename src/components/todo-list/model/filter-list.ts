...
const useFilteredList = (indexOption: number) => {
  const filterOption: OptionType = filterOptions.find((option: OptionType) => option.id === indexOption) ?? filterOptions[0]
  const filterConfig: ConfigType = filterOption.config
  
  const filteredList = todoStore.todos.map((todo: TodoType) => {
    return Object.keys(filterConfig).some((key: string) => todo[key as keyof TodoType] !== filterConfig[key as keyof ConfigType])
    ? null
